...
# или алгоритм Грэхема,
# или алгоритм Джарвиса,
# или метод «разделяй и властвуй»
import matplotlib.pyplot as plt


def rotate(point_A, point_B, point_C):  # функция определения положения точки C отн-но вектора AB
    # если результат < 0 - С справа, > 0 - С слева, = 0 - С на той же прямой, что и AB
    return (point_B[0] - point_A[0]) * (point_C[1] - point_B[1]) - (point_B[1] - point_A[1]) * (point_C[0] - point_B[0])
    # AB[0]*BC[1] - AB[1]BC[0]


def minimal_convex_hull(point_list):
    n = len(point_list)  # число точек
    points_numbers = list(range(n))  # список номеров точек

    for i in range(1, n):  # если points_numbers[i]-ая
        if point_list[points_numbers[i]][0] < point_list[points_numbers[0]][0]:  # точка лежит левее points_numbers[0]-ой
